    ...
    <div className="bg-slate-100 text-gray-900 min-h-screen flex flex-col justify-between font-sans antialiased [-webkit-font-smoothing:antialiased]">
      ...
      <header className="w-full bg-white/95 backdrop-blur-md border-b border-gray-200 sticky top-0 z-50 shadow-sm">
        ...
        <div className="max-w-[1360px] mx-auto px-10 py-3 flex justify-between items-center border-b border-gray-100 text-sm">
          ...
          <div className="flex items-center gap-5 text-gray-700 font-bold text-sm">
            ...
            <Link to="/setting" className="text-emerald-700 font-black transition">설정</Link>
      ...
      <main className="max-w-[900px] mx-auto px-6 py-10 w-full flex-1 space-y-8">
        ...
        <div className="mb-2 px-2">
          ...
          <p className="text-gray-700 font-bold text-base">계정 정보 관리 및 서비스 환경을 설정할 수 있습니다.</p>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-200 p-8 shadow-md space-y-6">
          <div className="flex justify-between items-center border-b border-gray-100 pb-4">
            ...
            <button type="button" className="px-5 py-2.5 bg-white border-2 border-gray-200 hover:bg-gray-50 text-gray-800 font-black text-xs rounded-2xl transition shadow-sm">
              회원수정
            </button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            <div className="space-y-1">
              <span className="text-gray-500 font-black text-xs block">아이디</span>
              <p className="text-gray-900 font-black text-base">smhrd</p>
            </div>
            <div className="space-y-1">
              <span className="text-gray-500 font-black text-xs block">이메일</span>
              <p className="text-gray-900 font-black text-base">smhrd123@gmail.com</p>
            </div>
            <div className="space-y-1">
              <span className="text-gray-500 font-black text-xs block">이름</span>
              <p className="text-gray-900 font-black text-base">홍길동</p>
            </div>
            <div className="space-y-1">
              <span className="text-gray-500 font-black text-xs block">성별</span>
              <p className="text-gray-900 font-black text-base">남성</p>
            </div>
            <div className="space-y-1">
              <span className="text-gray-500 font-black text-xs block">가입 생년월일</span>
              <p className="text-gray-900 font-black text-base">1985년 04월 12일</p>
            </div>
            <div className="space-y-1">
              <span className="text-gray-500 font-black text-xs block">휴대폰 번호</span>
              <p className="text-gray-900 font-black text-base">010-1234-5678</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-200 p-8 shadow-md space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <p className="font-black text-gray-900 text-base">로그아웃</p>
              <p className="text-gray-600 text-xs font-bold">현재 접속 중인 계정에서 안전하게 로그아웃합니다.</p>
            </div>
            <Link to="/login" className="w-full sm:w-auto px-6 py-3 border-2 border-gray-200 hover:bg-slate-50 rounded-2xl text-xs font-black text-gray-800 transition shadow-sm text-center">
              로그아웃하기
            </Link>
          </div>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-gray-100">
            <div>
              <p className="font-black text-red-600 text-base">회원 탈퇴</p>
              <p className="text-gray-600 text-xs font-bold">탈퇴 시 모든 주소록 및 이미지, 발송 내역 데이터가 영구적으로 삭제됩니다.</p>
            </div>
            <Link to="/start" className="w-full sm:w-auto px-6 py-3 border-2 border-red-200 hover:bg-red-50 text-red-600 rounded-2xl text-xs font-black transition shadow-sm text-center">
              회원 탈퇴하기
            </Link>
          </div>
        </div>
