    ...
    <div className="bg-slate-100 text-gray-900 min-h-screen flex flex-col justify-between font-sans antialiased [-webkit-font-smoothing:antialiased]">
        ...
        <div className="max-w-[1360px] mx-auto px-10 py-4 flex items-center justify-between">
          <div className="flex items-center gap-10">
            ...
            <nav className="flex items-center gap-8 font-bold text-base text-gray-700">
              ...
              <Link to="/createimage" className="text-emerald-800 border-b-2 border-emerald-800 py-1 transition">이미지 만들기</Link>
      ...
      <main className="max-w-[1360px] mx-auto px-10 py-10 w-full flex-1">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 font-medium text-base">상품 정보를 입력하면 AI가 자동으로 세련된 홍보 이미지를 만들어 드립니다.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-6 bg-white rounded-3xl border border-gray-200 p-8 shadow-sm flex flex-col justify-between space-y-6">
            <div className="space-y-6">
              <div>
                <h2 className="text-xl font-extrabold text-gray-900 mb-1">기본 상품 정보</h2>
                <p className="text-gray-500 font-medium text-sm">이미지에 들어갈 핵심 상품 정보를 입력해 주세요.</p>
              </div>

              {/* 상품명 */}
              <div className="space-y-2">
                <label className="block text-sm font-bold text-gray-800">상품명</label>
                <input 
                  type="text" 
              ...
              {/* 상품 특징 */}
              <div className="space-y-2">
                <label className="block text-sm font-bold text-gray-800">상품 특징</label>
                <input 
                  type="text" 
              ...
              {/* 상품 가격 */}
              <div className="space-y-2">
                <label className="block text-sm font-bold text-gray-800">상품 가격</label>
                <input 
                  type="text" 
              ...
              {/* 문의 연락처 */}
              <div className="space-y-2">
                <label className="block text-sm font-bold text-gray-800">문의 연락처</label>
                <input 
                  type="text" 
              ...
              {/* 참고 이미지 업로드 (선택) */}
              <div className="space-y-2">
                <label className="block text-sm font-bold text-gray-800">참고 이미지 업로드 <span className="text-gray-400 font-normal">(선택)</span></label>
                <div className="border-2 border-dashed border-emerald-300 bg-emerald-50/50 rounded-2xl p-6 text-center hover:bg-emerald-50 transition cursor-pointer flex flex-col items-center justify-center gap-2">
                  <span className="text-3xl text-emerald-700 mb-1">📁</span>
                  <p className="text-sm font-bold text-gray-700">여기에 이미지를 드래그하거나 클릭하여 업로드</p>
                  <p className="text-xs text-gray-400 font-medium">JPG, PNG 파일 지원 (최대 10MB)</p>
                </div>
              </div>
            </div>

            {/* 생성 버튼 */}
            <button type="button" className="w-full py-4 bg-emerald-800 hover:bg-emerald-900 text-white font-extrabold text-xl rounded-2xl shadow-md transition-all hover:shadow-lg active:scale-[0.99] flex items-center justify-center gap-3 mt-6">
              AI 이미지 생성하기
            </button>
          </div>
          ...
          <div className="lg:col-span-6 flex flex-col justify-between space-y-6">
            <div className="bg-white rounded-3xl border border-gray-200 p-8 shadow-sm space-y-6">
              <div>
                <h2 className="text-xl font-extrabold text-gray-900 mb-1">추가 요청사항 입력</h2>
                <p className="text-gray-500 font-medium text-sm">이미지에 반영할 내용을 자유롭게 입력해 주세요. 왼쪽 기본 정보 외에 AI에게 추가로 전달할 내용입니다.</p>
              </div>

              {/* 예시 프롬프트 칩 목록 */}
              <div className="space-y-3">
                <p className="text-sm font-bold text-gray-700">이런 느낌으로 작성해 보세요! <span className="text-xs text-emerald-700 font-semibold">(클릭하면 자동으로 입력됩니다)</span></p>
                
                <div className="p-4 bg-emerald-50/60 border border-emerald-200/70 rounded-2xl text-sm font-semibold text-gray-700 cursor-pointer hover:bg-emerald-100/60 transition">
                  [예시 1] 비료 포대가 논밭에 놓여 있고 글씨가 매우 크게 보이게 해주세요.
                </div>
                <div className="p-4 bg-emerald-50/60 border border-emerald-200/70 rounded-2xl text-sm font-semibold text-gray-700 cursor-pointer hover:bg-emerald-100/60 transition">
                  [예시 2] 2027년 정부 보조금 지원 문구가 잘 보이도록 밝은 배경으로 만들어 주세요.
                </div>
                <div className="p-4 bg-emerald-50/60 border border-emerald-200/70 rounded-2xl text-sm font-semibold text-gray-700 cursor-pointer hover:bg-emerald-100/60 transition">
              ...
              {/* 자유 입력 (프롬프트) */}
              <div className="space-y-2">
                <label className="block text-sm font-bold text-gray-800">자유 입력 (프롬프트)</label>
                <div className="relative">
                  <textarea 
                    rows="7" 
                    placeholder="예) 비료 포대가 논 위에 놓여 있고, 글씨 크게 가격이 강조된 이미지로 만들어 주세요. 배경은 초록색 계열로 해 주세요." 
                    className="w-full px-4 py-3.5 text-sm border-2 border-gray-200 rounded-2xl focus:outline-none focus:border-emerald-700 focus:ring-2 focus:ring-emerald-100 transition-all font-medium text-gray-800 resize-none pb-8"
                  ></textarea>
                  <div className="absolute bottom-3 right-4 text-xs font-semibold text-gray-400">0자</div>
                </div>
              </div>
            </div>

            {/* 안내 박스 */}
            <div className="bg-amber-50/80 border border-amber-200/80 rounded-3xl p-7 text-amber-900 space-y-3 shadow-sm">
              <h3 className="font-extrabold text-base flex items-center gap-2">
                <span className="text-amber-600 text-lg">⚠️</span> 작성에 도움이 필요하신가요?
              </h3>
